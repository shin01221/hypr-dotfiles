...
# TODO Cleanup this code to make it simpler
...
source "/home/shin/.local/lib/hyde/globalcontrol.sh"
...
gentheme() {
    ...
    gencolors | cat - <(echo "$CONSTANT_STRING") >"$tempdir/colors.tdesktop-theme"

	if command -v zip >/dev/null 2>&1; then
		if [ "$walmode" = "solid" ]; then
			# $background is set by wal in colors.sh template
			convert -size 256x256 "xc:${bgcolor:-${background:-$color0}}" "$tempdir/$walname" 2>/dev/null
		else
			case "$(file -b --mime-type "$wal")" in
			image/*) convert -resize 1920x1080 ${blur:+-blur 0x16} "$wal" "$tempdir/$walname" 2>/dev/null ;;
			*) error "not an image: $wal" ;;
			esac
		fi
		# syncing files (with '-FS') in an existing archive is faster than creating new
		zip -j -FS "$landingDir/$themename" "$tempdir"/* >/dev/null 2>&1 || {
			# create clean zip in case syncing failed
			rm -f "$landingDir/$themename"
			zip -jq "$landingDir/$themename" "$tempdir"/* >/dev/null 2>&1
		}
	else
		echo "[wallbash] 'zip' not found. theme generated without background image"
		cp -f "$tempdir/colors.tdesktop-theme" "$landingDir/$themename"
	fi
}

main() {
	walmode="background"
	theme="${cacheDir}/landing/wallbash-telegram.sh"
	[ -f "$walfile" ] && read -r wal <"$walfile"

	while getopts ":vhi:f:b:Bcs" opt; do
		case "$opt" in
		v)
			printf "This is a modified version of walogram that works for HyDE\\n"
			exit 0
			;;
		h)
			echo "This is a modified version of walogram that works for HyDE"
            echo "Visit https://codeberg.org/thirtysix/walogram/src/branch/master/walogram for the original version"
			exit 0
			;;
		b)
		...
		f) theme="$OPTARG" ;;
		B) blur="true" ;;
		c) rm -f "$landingDir"/* && echo "cache cleared" ;;
		s) walmode="solid" ;;
		:) error "'-$OPTARG' requires argument" ;;
		\?) error "invalid option: '-$OPTARG'" ;;
		esac
	done
    wal="${cacheDir}/wall.blur"
	...
	for i in 0 1 2 3 4 5 6 7; do
		...
		[ -z "$(eval "echo \"\$$bright\"")" ] && {
			echo "$normal value used for $bright, because it is unset"
			eval "$bright=\$$normal"
		}
	done
...
//   Doing copy paste from telegram official documentation is not
//   a good idea because IMHO, because it doesn't explain anything.
// - Remove unused alpha colors.
// - Add the missing alpha colors.
// - Fix colors marked with [UNTESTED].
...
windowSubTextFg: colorDarker7_20;                            // testo in basso a sinistra nel menu sinistra + testo messo in meno risalto nel menu opzioni
...
msgInMonoFg: colorLighter3_20;                               // inbox mex monospace non selezionato
msgInMonoFgSelected: colorLighter7_40;                       // inbox mex monospace selezionato
msgOutMonoFg: colorLighter3_20;                              // outbox mex monospace non selezionato
